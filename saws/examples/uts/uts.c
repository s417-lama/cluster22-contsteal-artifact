...
int uts_paramsToStr(char *strBuf, int ind) {
  ...
  if (type == BALANCED) {
    ...
    ind += sprintf(strBuf+ind, "      Expected size: %ld nodes, %ld leaves\n",
...
void uts_showStats(int nPes, int chunkSize, uint64_t walltime, counter_t nNodes, counter_t nLeaves, counter_t maxDepth) {
  // summarize execution info for machine consumption
  if (verbose == 0) {
    printf("%4d %ld %9ld %.6g %.6g %d %d %.2f %d %d %1d %f %3d\n",
        nPes, walltime, nNodes, (double)nNodes/walltime, (double)nNodes/walltime/nPes, chunkSize, 
        type, b_0, rootId, gen_mx, shape_fn, nonLeafProb, nonLeafBF);
  }

  // summarize execution info for human consumption
  else {
    printf("Tree size = %ld, tree depth = %ld, num leaves = %ld (%.2f%%)\n", nNodes, maxDepth, nLeaves, nLeaves/(float)nNodes*100.0); 
    printf("Wallclock time = %ld ns, performance = %.6g Gnodes/sec (%.6g Gnodes/sec per PE)\n\n",
        walltime, (double)nNodes / walltime, (double)nNodes / walltime / nPes);
  }
}
