...
uint64_t ldbal_scioto(UTSIterator iter) {
#ifdef STRICT_DFS
...
#define INITIAL_TASKS nproc
    ...
    double perf = (double)nNodes / process_time;

    if (me == 0) {
      printf("[%d] %ld ns %.6g Gnodes/s ( nodes: %ld depth: %ld leaves: %ld )\n",
             i, process_time, perf, nNodes, maxDepth, nLeaves);
    }
