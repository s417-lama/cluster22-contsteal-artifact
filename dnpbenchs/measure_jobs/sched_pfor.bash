...
# MEASURE_CONFIG=${MEASURE_CONFIG:-mpi}

DNP_PREFIX=sched_pfor_${MEASURE_PATTERN}_${MEASURE_CONFIG}
...
dnp_run() {
  ...
  case $MEASURE_PATTERN in
    strong)
      NODEs=(1 2 4 8 16)
      M=10000
      NMs=(65536,$M 131072,$M 262144,$M 524288,$M 1048576,$M 2097152,$M 4194304,$M 8388608,$M)
      ;;
    size_n)
      NODEs=($n_nodes)
      M=10000
      # NMs=($(for n in $(seq 0 7); do echo $((16384 * (2 ** n) * $n_nodes)),$M; done))
      NMs=($(for n in $(seq 0 9); do echo $((4096 * (2 ** n) * $n_nodes)),$M; done))
      ;;
    size_nm)
      NODEs=($n_nodes)
      NMs=($(
        M=1000
        # for n in $(seq 0 7); do echo $((131072 * (2 ** n) * $n_nodes)),$M; done
        for n in $(seq 0 9); do echo $((32768 * (2 ** n) * $n_nodes)),$M; done
        M=2000
        # for n in $(seq 0 7); do echo $((65536 * (2 ** n) * $n_nodes)),$M; done
        for n in $(seq 0 9); do echo $((16384 * (2 ** n) * $n_nodes)),$M; done
        M=5000
        # for n in $(seq 0 7); do echo $((32768 * (2 ** n) * $n_nodes)),$M; done
        for n in $(seq 0 9); do echo $((8192 * (2 ** n) * $n_nodes)),$M; done
        M=10000
        # for n in $(seq 0 7); do echo $((16384 * (2 ** n) * $n_nodes)),$M; done
        for n in $(seq 0 9); do echo $((4096 * (2 ** n) * $n_nodes)),$M; done
        M=100000
        # for n in $(seq 0 7); do echo $((2048 * (2 ** n) * $n_nodes)),$M; done
        for n in $(seq 0 9); do echo $((512 * (2 ** n) * $n_nodes)),$M; done
      ))
      ;;
  esac
  ...
  for node in "${NODEs[@]}"; do
    for nm in "${NMs[@]}"; do
      ...
      for i in $(seq 1 $REPEAT_OUTSIDE); do
        ...
        if [[ $MEASURE_CONFIG == mpi ]]; then
          dnp_mpirun $((node * DNP_CORES)) $DNP_CORES $out_file ./sched_bench_mpi.out -n $n -m $m -b $N_BLOCKS -r $REPEAT_INSIDE -f $CPU_SIMD_FREQ -w $N_WARMUP
        else
          dnp_mpirun $((node * DNP_CORES)) $DNP_CORES $out_file ./sched_bench.out -c pfor -n $n -m $m -b $N_BLOCKS -r $REPEAT_INSIDE -f $CPU_SIMD_FREQ -w $N_WARMUP
        fi
      done
