...
void SearchConductor::start(int nrepeats = 1)
{
    numRepeats = nrepeats;
    initialNumRepeats = nrepeats;
    groupInitCount = 0;
    groupProxy.init();
}
...
void SearchConductor::allSearchNodeDone( CkQdMsg *msg )
{
    ...
    if( numSolutions > 0 )
    {
        /* CkPrintf( "%lld solutions are found in %lf sec, with %d processors\n", numSolutions, uts_wctime()-startTime, CkNumPes() ); */	

        uint64_t exec_time = uts_wctime() - startTime;
        double perf = (double)numSolutions / exec_time;
        CkPrintf("[%d] %ld ns %.6g Gnodes/s ( nodes: %ld )\n",
                 initialNumRepeats - numRepeats, exec_time, perf, numSolutions);
