...
int shape_fn = LINEAR; // default shape function (b_i decr linearly)
...
int uts_paramsToStr(char *strBuf, int ind) {
  ...
  ind += sprintf(strBuf+ind, "UTS - Unbalanced Tree Search %s (Charm++)\n", UTS_VERSION);
...
void uts_parseParams(int argc, char *argv[]){
  ...
  int err = -1;
  while (i < argc && err == -1) {
    if (argv[i][0] == 'M' && argv[i][1] == 'h') {
        uts_helpMessage();

    } else if (argv[i][0] != 'M' || strlen(argv[i]) != 2 || argc <= i+1) {
      err = i; break;
    }
    ...
    switch (argv[i][1]) {
      ...
      case 'g':
        computeGranularity = __max(1,atoi(argv[i+1])); break;
      default:
        err = i;
    }
...
void uts_helpMessage() {
  printf("  UTS - Unbalanced Tree Search %s (Charm++)\n\n", UTS_VERSION);
